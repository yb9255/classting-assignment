...
차트를 만드는데 D3를 사용하였는데, D3 라이브러리 특성상 여러 메소드가 체이닝으로 되어 있어 메소드를 모킹하기가 어려워 제대로 된 테스트가 작성되지 않은 것 같아 아쉽습니다. 차후 메소드 체이닝이 많은 라이브러리의 경우 어떤식으로 테스트를 적용할 수 있는지 더 공부해야 겠다는 생각이 들었습니다.
